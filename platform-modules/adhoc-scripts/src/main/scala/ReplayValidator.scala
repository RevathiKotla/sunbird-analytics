...
object ReplayValidator extends optional.Application {

    def main(fromDate: String, toDate: String)(implicit sc: SparkContext = null) {

        if (null == sc) {
            implicit val sc: SparkContext = CommonUtil.getSparkContext(10, "ProcessFailedEvents");
            correctData(fromDate, toDate);
        }
        else {
            correctData(fromDate, toDate);
        }
    ...
    def correctData(fromDate: String, toDate: String)(implicit sc: SparkContext): Unit = {
